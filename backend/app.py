...
def get_next_question(quiz_id):
    ...
    answered_ids = progress.question_history or []
    ...
    # Record that this question is served to this user/session to avoid repeats
    if question.id not in progress.question_history:
        progress.question_history.append(question.id)
    progress.active_session_answers = len(progress.question_history or [])
    db.session.commit()
